...
const questions = (filename, defaultData) => ([
  {
    type: 'list',
    name: 'type',
  ...
  },
  {
    ...
    default: function() {
      const output = fs.readFileSync(path.resolve(process.cwd(), filename, config.gitHead), 'utf-8');
    ...
    validate: function(version) {
      if(/^([0-9]{1,2}\.){2}[0-9]{1,2}$/.test(version)) {
        ...
        try {
          // git文件取branch得优化下。
          files = fs.readdirSync(path.resolve(process.cwd(), filename, config.gitDirBranch));
        } catch(err) {
