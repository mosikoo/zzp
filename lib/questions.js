...
const questions = (filename, defaultData, isCurrentDir) => ([
  ...
  },
  {
    ...
    default: function() {
      ...
      const output = fs.readFileSync(path.resolve(process.cwd(), _filename, utils.gitHead), 'utf-8');
    ...
    validate: function(version) {
      if(/^([0-9]{1,2}\.){2}[0-9]{1,2}$/.test(version)) {
        ...
        try {
          const _filename = isCurrentDir ? '' : filename;
          files = fs.readdirSync(path.resolve(process.cwd(), _filename, utils.gitDirBranch));
        } catch(err) {
...
const confirmQuestion = [{
  ...
  name: 'isOk',
  message: 'The above project you want to publish?'
}]
